...
class Client {
    ...
    async fetchCategories() {
        const request = await this.requester.request('https://plus.character.ai/chat/character/categories/')

        if (request.status() === 200) return await Parser.parseJSON(request);
        else throw Error('Failed to fetch categories');
    }
    async fetchUserConfig() {
        const request = await this.requester.request('https://plus.character.ai/chat/config/', {
    ...
    async fetchUser() {
        ...
        const request = await this.requester.request('https://plus.character.ai/chat/user/', {
    ...
    async fetchFeaturedCharacters() {
        ...
        const request = await this.requester.request('https://plus.character.ai/chat/characters/featured_v2/', {
            headers:this.getHeaders()
        })
    ...
    async fetchCharactersByCategory(curated = false) {
        ...
        const url = `https://plus.character.ai/chat/${
    ...
    async fetchCharacterInfo(characterId) {
        ...
        const request = await this.requester.request(`https://plus.character.ai/chat/character/info-cached/${characterId}/`, {
            headers:this.getHeaders(),
        })
    ...
    async searchCharacters(characterName) {
        ...
        const request = await this.requester.request(`https://plus.character.ai/chat/characters/search/?query=${characterName}`, {
            headers:this.getHeaders()
        })
    ...
    async getRecentConversations() {
        if (!this.isAuthenticated()) throw Error('You must be authenticated to do this.');
        const request = await this.requester.request(`https://plus.character.ai/chat/characters/recent/`, {
            headers:this.getHeaders()
        })
    ...
    async createOrContinueChat(characterId, externalId = null) {
        ...
        let request = await this.requester.request('https://plus.character.ai/chat/history/continue/',  {
        ...
        if (request.status() === 200 || request.status() === 404) {
            ...
            if (response === "No Such History" || response === "there is no history between user and character") { // Create a new chat
                request = await this.requester.request('https://plus.character.ai/chat/history/create/', {
                    body:Parser.stringify({
                        character_external_id: characterId,
    ...
    async authenticateWithToken(token) {
        ...
        const request = await this.requester.request('https://plus.character.ai/dj-rest-auth/auth0/', {
            method:'POST',
            body:Parser.stringify({
                access_token: token
            }),
    ...
    async authenticateAsGuest() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const baseRequest = await Promise.race([
                this.requester.request('https://plus.character.ai/chat/auth/lazy/', {
                    method: 'POST',
                    body: payload,
                    headers: this.#guestHeaders,
                }),
