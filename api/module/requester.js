...
class Requester {
    ...
    async request(url, options) {
        ...
        if (this.usePlus) // Props to @Kaidesa
            url = url.replace('plus.character.ai', 'plus.character.ai');
        ...
        try {
            ...
            const authenticating = (url == "https://plus.character.ai/chat/auth/lazy/")
        ...
        try {
            ...
            response = await page.evaluate(
                async (headers, buffer) => {
                        ...
                        const uploadResponse = await fetch("https://plus.character.ai/chat/upload-image/", {
                            headers: headers,
                            method: "POST",
                            body: formData
                        })
